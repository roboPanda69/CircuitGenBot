...
KiCad net labels are attached at actual generated pin endpoints, and no-connect markers are attached to exact no-connect endpoints. `KiCadArtifactChecker` requires one complete top-level document, validates KiCad 9 root structure/version/UUIDs/sheet instances/symbol instances, requires coherent placed-symbol/path/pin records for every represented component, models local and global electrical labels separately from annotation text, accounts for every raw no-connect marker, proves unresolved pins are electrically unattached, reconstructs the artifact graph, and gates `GenerationResult.status`.
